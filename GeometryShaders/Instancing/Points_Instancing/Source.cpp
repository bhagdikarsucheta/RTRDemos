...
void initialize()
{
	...
	const GLchar *geometryShaderSource =
		...
		"gl_Position = u_mvp_matrix * (gl_in[0].gl_Position )+vec4(-0.1,0.0,0.0,0.0);"\
		"EmitVertex();"\
		"gl_Position = u_mvp_matrix * (gl_in[0].gl_Position )+vec4(0.1,0.0,0.0,0.0);"\
		"EmitVertex();"\
	...
	//glGenBuffers(1, &gVboColor2);
	//glBindBuffer(GL_ARRAY_BUFFER, gVboColor2);
	//glBufferData(GL_ARRAY_BUFFER, sizeof(colorVertices2), colorVertices2, GL_STATIC_DRAW);
