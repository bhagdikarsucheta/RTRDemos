...
*/
	...
	glGenBuffers(1, &gVbo_Indices);
	glBindBuffer(GL_ARRAY_BUFFER, gVbo_Indices);
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertex_indices), vertex_indices, GL_STATIC_DRAW);
...
void display(void)
{
	...
	modelviewMatrix = translate(0.0f, 0.0f, -3.0f)* vmath::translate(0.0f, // sinf(2.1f * f) * 0.5f,
	...
	glDrawElements(GL_PATCHES, 24, GL_UNSIGNED_BYTE, 0);
	//(each with its x,y,z ) vertices in triangleVertices array
...
void resize(int width, int height)
{
	...
	gPerspectiveProjectionMatrix = perspective(50.0f, (GLfloat)width / (GLfloat)height, 0.1f, 100.0f);
}
