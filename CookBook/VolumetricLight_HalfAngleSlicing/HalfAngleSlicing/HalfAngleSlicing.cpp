...
GLuint gVertexShaderObjectShadow;
GLuint gFragmentShaderObjectShadow;
GLuint gShaderProgramObjectShadow;

GLuint gVertexShaderObjectflat;
GLuint gFragmentShaderObjectflat;
GLuint gShaderProgramObjectflat;
...
//volume data files 
const std::string volume_file = "E:/RealTimeRendering2018/Trial_VolumetricLight_HalfAngleSlicing/Trial_Volume_Rendering/08_SHADERS_3D_PYRAMID_CUBE/Engine256.raw";
...
GLuint gMVPUniformShadow,ShadowTex,colorUniformShadow,SUniform,volumeUniform;
GLuint gMVPUniformflat;
GLuint gMVPUniformQuad,gTextureMapUniform;
...
//light vertex array and buffer object ID's
GLuint lightVAOID;
GLuint lightVerticesVBO;
//Object space light position
glm::vec3 lightPosOS = glm::vec3(0, 5, 0);

//lights spherical coordinates
float theta = 0.66f;
float phi = -1.0f;
float radius = 2;
...
const int DOWN_SAMPLES = 2;

//down sampled image width and height
const int IMAGE_WIDTH = WIN_WIDTH / DOWN_SAMPLES;
const int IMAGE_HEIGHT = WIN_HEIGHT / DOWN_SAMPLES;


glm::mat4 MV_L;	//light model view matrix
glm::mat4 P_L;	//light projection matrix
glm::mat4 B;	//light bias matrix
glm::mat4 BP;		//lights bias and projection matrix combined
glm::mat4 S;		//lights combined BPMV matrix
...
//color of light attenuation
glm::vec3 lightAttenuation = glm::vec3(0.1, 0.2, 0.3);

//shadow alpha value
float fShadowAlpha = 1;

//attachment ID's for FBO colour attachments
...
//format (internal format) and format(format)
//The function returns the texture ID of newly created texture.
...
void InitFBO()
{
	...
	if (status == GL_FRAMEBUFFER_COMPLETE)
		fprintf(gpFile, "Light FBO Setup successfull!!");
	else
		fprintf(gpFile, "Problem with Light FBO setup!!");
...
void SliceVolume()
{
	...
	for (int i = 0; i < 12; i++)
	{
		...
		//do a lot of vecDir with the halfVec direction vector
...
void initialize()
{
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"void main (void)" \
		"{" \
			"FragColor = texture(volume,vUV).rrrr*color;" \
		"}";
	...
	/////////////////////////////////////////Load Slicing Shadow Shader///////////////////////////



	//******** VERTEX SHADER ******
	//create shader
	gVertexShaderObjectShadow = glCreateShader(GL_VERTEX_SHADER);

	//provide source code to shader
	const GLchar *vertexShaderSourceCodeShadow =
		"#version 330 core" \
		"\n" \
		"in vec3 vPosition;" \
		"uniform mat4 u_mvp_matrix;" \
		...
		"void main(void)" \
		"{" \
			"vLightUVW = S*vec4(vPosition,1);"\
			"gl_Position = u_mvp_matrix * vec4(vPosition,1);" \
			"vUV = vPosition + vec3(0.5);"\
		"}";

	glShaderSource(gVertexShaderObjectShadow, 1, (const GLchar **)&vertexShaderSourceCodeShadow, NULL);

	//compile shader
	glCompileShader(gVertexShaderObjectShadow);
	 iInfoLength = 0;
	 iShaderCompiledStatus = 0;
	 szInfoLog = NULL;

	glGetShaderiv(gVertexShaderObjectShadow, GL_COMPILE_STATUS, &iShaderCompiledStatus);
	if (iShaderCompiledStatus == GL_FALSE)
	{
		glGetShaderiv(gVertexShaderObjectShadow, GL_INFO_LOG_LENGTH, &iInfoLength);
		if (iInfoLength > 0)
		{
			szInfoLog = (char *)malloc(iInfoLength);
			if (szInfoLog != NULL)
			{
				GLsizei written;
				glGetShaderInfoLog(gVertexShaderObjectShadow, iInfoLength, &written, szInfoLog);
				fprintf(gpFile, "Vertex Shader Compilation Log volume shader:%s\n", szInfoLog);
				free(szInfoLog);
				uninitialize();
				exit(0);
	...
	//**********FRAGMENT SHADER*********
	//create shader
	gFragmentShaderObjectShadow = glCreateShader(GL_FRAGMENT_SHADER);

	//provide source code to shader
	const GLchar *fragmentShaderSourceCodeShadow =
		"#version 330 core" \
		"\n" \
		"smooth in vec3 vUV;" \
		"out vec4 FragColor;" \
		...
		"{" \
			...
			"{"\
				...
				"FragColor = vec4(color.xyz*LightIntensity*alpha,alpha);" \
			"}"\
		"}";

	glShaderSource(gFragmentShaderObjectShadow, 1, (const GLchar**)&fragmentShaderSourceCodeShadow, NULL);

	//compile shader
	glCompileShader(gFragmentShaderObjectShadow);
	glGetShaderiv(gFragmentShaderObjectShadow, GL_COMPILE_STATUS, &iShaderCompiledStatus);
	if (iShaderCompiledStatus == GL_FALSE)
	{
		glGetShaderiv(gFragmentShaderObjectShadow, GL_INFO_LOG_LENGTH, &iShaderCompiledStatus);
		if (iInfoLength > 0)
		{
			szInfoLog = (char *)malloc(iInfoLength);
			if (szInfoLog != NULL)
			{
				GLsizei written;
				glGetShaderInfoLog(gFragmentShaderObjectShadow, iInfoLength, &written, szInfoLog);
				fprintf(gpFile, "Fragment Shader Compilation Log Volume shader: %s\n", szInfoLog);
				free(szInfoLog);
				uninitialize();
				exit(0);
	...
	//********** SHADER PROGRAM *********
	//create
	gShaderProgramObjectShadow = glCreateProgram();

	//attach vertex shader to shader program
	glAttachShader(gShaderProgramObjectShadow, gVertexShaderObjectShadow);

	//attach fragment shader to shader program
	glAttachShader(gShaderProgramObjectShadow, gFragmentShaderObjectShadow);

	//pre-link binding of shader program object with vertex position attribute
	glBindAttribLocation(gShaderProgramObjectShadow, VDG_ATTRIBUTE_VERTEX, "vPosition");


	//Link Shader
	glLinkProgram(gShaderProgramObjectShadow);
	 iShaderProgramLinkStatus = 0;
	glGetProgramiv(gShaderProgramObjectShadow, GL_LINK_STATUS, &iShaderProgramLinkStatus);
	if (iShaderProgramLinkStatus == GL_FALSE)
	{
		glGetProgramiv(gShaderProgramObjectShadow, GL_INFO_LOG_LENGTH, &iInfoLength);
		if (iInfoLength > 0)
		{
			szInfoLog = (char *)malloc(iInfoLength);
			if (szInfoLog != NULL)
			{
				GLsizei written;
				glGetProgramInfoLog(gShaderProgramObjectShadow, iInfoLength, &written, szInfoLog);
				fprintf(gpFile, "Shader Program LinK Log Volume Shader %s\n");
				free(szInfoLog);
				uninitialize();
				exit(0);
	...
	gMVPUniformShadow = glGetUniformLocation(gShaderProgramObjectShadow, "u_mvp_matrix");
	volumeUniform = glGetUniformLocation(gShaderProgramObjectShadow, "volume");
	colorUniformShadow = glGetUniformLocation(gShaderProgramObjectShadow, "color");
	ShadowTex = glGetUniformLocation(gShaderProgramObjectShadow, "shadowTex");
	SUniform = glGetUniformLocation(gShaderProgramObjectShadow, "S");
	...
	//******** VERTEX SHADER ******
	//create shader
	gVertexShaderObjectflat = glCreateShader(GL_VERTEX_SHADER);

	//provide source code to shader
	const GLchar *vertexShaderSourceCodeflat =
		"#version 330 core" \
		"\n" \
		"in vec3 vPosition;" \
		"uniform mat4 u_mvp_matrix;" \
	...
	glShaderSource(gVertexShaderObjectflat, 1, (const GLchar **)&vertexShaderSourceCodeflat, NULL);

	//compile shader
	glCompileShader(gVertexShaderObjectflat);
	 iInfoLength = 0;
	 iShaderCompiledStatus = 0;
	 szInfoLog = NULL;

	glGetShaderiv(gVertexShaderObjectflat, GL_COMPILE_STATUS, &iShaderCompiledStatus);
	if (iShaderCompiledStatus == GL_FALSE)
	{
		glGetShaderiv(gVertexShaderObjectflat, GL_INFO_LOG_LENGTH, &iInfoLength);
		if (iInfoLength > 0)
		{
			szInfoLog = (char *)malloc(iInfoLength);
			if (szInfoLog != NULL)
			{
				GLsizei written;
				glGetShaderInfoLog(gVertexShaderObjectflat, iInfoLength, &written, szInfoLog);
				fprintf(gpFile, "Vertex Shader Compilation Log volume shader:%s\n", szInfoLog);
				free(szInfoLog);
				uninitialize();
				exit(0);
	...
	//**********FRAGMENT SHADER*********
	//create shader
	gFragmentShaderObjectflat = glCreateShader(GL_FRAGMENT_SHADER);

	//provide source code to shader
	const GLchar *fragmentShaderSourceCodeflat =
		"#version 330 core" \
		"\n" \
		"out vec4 FragColor;" \
	...
	glShaderSource(gFragmentShaderObjectflat, 1, (const GLchar**)&fragmentShaderSourceCodeflat, NULL);

	//compile shader
	glCompileShader(gFragmentShaderObjectflat);
	glGetShaderiv(gFragmentShaderObjectflat, GL_COMPILE_STATUS, &iShaderCompiledStatus);
	if (iShaderCompiledStatus == GL_FALSE)
	{
		glGetShaderiv(gFragmentShaderObjectflat, GL_INFO_LOG_LENGTH, &iShaderCompiledStatus);
		if (iInfoLength > 0)
		{
			szInfoLog = (char *)malloc(iInfoLength);
			if (szInfoLog != NULL)
			{
				GLsizei written;
				glGetShaderInfoLog(gFragmentShaderObjectflat, iInfoLength, &written, szInfoLog);
				fprintf(gpFile, "Fragment Shader Compilation Log Volume shader: %s\n", szInfoLog);
				free(szInfoLog);
				uninitialize();
				exit(0);
	...
	//********** SHADER PROGRAM *********
	//create
	gShaderProgramObjectflat = glCreateProgram();

	//attach vertex shader to shader program
	glAttachShader(gShaderProgramObjectflat, gVertexShaderObjectflat);

	//attach fragment shader to shader program
	glAttachShader(gShaderProgramObjectflat, gFragmentShaderObjectflat);

	//pre-link binding of shader program object with vertex position attribute
	glBindAttribLocation(gShaderProgramObjectflat, VDG_ATTRIBUTE_VERTEX, "vPosition");


	//Link Shader
	glLinkProgram(gShaderProgramObjectflat);
	 iShaderProgramLinkStatus = 0;
	glGetProgramiv(gShaderProgramObjectflat, GL_LINK_STATUS, &iShaderProgramLinkStatus);
	if (iShaderProgramLinkStatus == GL_FALSE)
	{
		glGetProgramiv(gShaderProgramObjectflat, GL_INFO_LOG_LENGTH, &iInfoLength);
		if (iInfoLength > 0)
		{
			szInfoLog = (char *)malloc(iInfoLength);
			if (szInfoLog != NULL)
			{
				GLsizei written;
				glGetProgramInfoLog(gShaderProgramObjectflat, iInfoLength, &written, szInfoLog);
				fprintf(gpFile, "Shader Program LinK Log Volume Shader %s\n");
				free(szInfoLog);
				uninitialize();
				exit(0);
	...
	gMVPUniformflat = glGetUniformLocation(gShaderProgramObjectflat, "u_mvp_matrix");
	...
	const GLchar *vertexShaderSourceCodeQuad =
		...
		"void main(void)" \
		"{" \
			"gl_Position =  vec4(vPosition.xy*2.0-1.0,0,1);" \
			"vUV = vPosition;"\
		"}";
	...
	glGetShaderiv(gVertexShaderObjectQuad, GL_COMPILE_STATUS, &iShaderCompiledStatus);
	if (iShaderCompiledStatus == GL_FALSE)
	{
		glGetShaderiv(gVertexShaderObjectQuad, GL_INFO_LOG_LENGTH, &iInfoLength);
		if (iInfoLength > 0)
		{
			szInfoLog = (char *)malloc(iInfoLength);
			if (szInfoLog != NULL)
			{
				GLsizei written;
				glGetShaderInfoLog(gVertexShaderObjectQuad, iInfoLength, &written, szInfoLog);
				fprintf(gpFile, "Vertex Shader Compilation Log volume shader:%s\n", szInfoLog);
				free(szInfoLog);
				uninitialize();
				exit(0);
	...
	glGetShaderiv(gFragmentShaderObjectQuad, GL_COMPILE_STATUS, &iShaderCompiledStatus);
	if (iShaderCompiledStatus == GL_FALSE)
	{
		glGetShaderiv(gFragmentShaderObjectQuad, GL_INFO_LOG_LENGTH, &iShaderCompiledStatus);
		if (iInfoLength > 0)
		{
			szInfoLog = (char *)malloc(iInfoLength);
			if (szInfoLog != NULL)
			{
				GLsizei written;
				glGetShaderInfoLog(gFragmentShaderObjectQuad, iInfoLength, &written, szInfoLog);
				fprintf(gpFile, "Fragment Shader Compilation Log Volume shader: %s\n", szInfoLog);
				free(szInfoLog);
				uninitialize();
				exit(0);
	...
	if (iShaderProgramLinkStatus == GL_FALSE)
	{
		glGetProgramiv(gShaderProgramObjectQuad, GL_INFO_LOG_LENGTH, &iInfoLength);
		if (iInfoLength > 0)
		{
			szInfoLog = (char *)malloc(iInfoLength);
			if (szInfoLog != NULL)
			{
				GLsizei written;
				glGetProgramInfoLog(gShaderProgramObjectQuad, iInfoLength, &written, szInfoLog);
				fprintf(gpFile, "Shader Program LinK Log Volume Shader %s\n");
				free(szInfoLog);
				uninitialize();
				exit(0);
...
//	gMVPUniformQuad = glGetUniformLocation(gShaderProgramObjectQuad, "u_mvp_matrix");
	gTextureMapUniform = glGetUniformLocation(gShaderProgramObjectQuad, "textureMap");
	...
	glBindVertexArray(0);
	///////////////////////////////////////////setup vao vbo stuff for the light position crosshair //////////////////////////
	...
	//enable the vertex attribute array for position
	glEnableVertexAttribArray(VDG_ATTRIBUTE_VERTEX);
	glVertexAttribPointer(VDG_ATTRIBUTE_VERTEX, 2, GL_FLOAT, GL_FALSE, 0, 0);

	//get the object space light position
	...
	//SET LIGHT mv, p and bias matrices
	MV_L = glm::lookAt(lightPosOS, glm::vec3(0, 0, 0), glm::vec3(0, 1, 0));
	P_L = glm::perspective(glm::radians(55.0f), 1.0f, 1.0f, 200.0f);
...
//function to render slices from the point of view of ight in the light buffer
...
void DrawSlices(const glm::mat4& MVP)
{
	...
	{
		glUseProgram(gShaderProgramObjectShadow);
		glUniform1i(volumeUniform, 0);
		glUniform1i(ShadowTex, 1);
		glUniform4f(colorUniformShadow, lightColor.x, lightColor.y, lightColor.z, lightColor.w);

		glUniformMatrix4fv(gMVPUniformShadow, 1, GL_FALSE, glm::value_ptr(MVP));
		glUniformMatrix4fv(SUniform, 1, GL_FALSE, glm::value_ptr(S));
		...
		glUniform4f(colorUniform, lightAttenuation.x*fShadowAlpha, lightAttenuation.y*fShadowAlpha, lightAttenuation.z*fShadowAlpha, 1);


		glUniformMatrix4fv(gMVPUniform, 1, GL_FALSE, glm::value_ptr(P_L*MV_L));
	...
	glUniform1i(gTextureMapUniform, 1);
...
void display(void)
{
	...
	glBindVertexArray(lightVAOID); {
		...
		glUseProgram(gShaderProgramObjectflat);
		glUniformMatrix4fv(gMVPUniformflat, 1, GL_FALSE, glm::value_ptr(P*MV*T));
...
void resize(int width, int height)
{
	...
	P = glm::perspective(glm::radians(60.0f), (GLfloat)width / (GLfloat)height, 0.1f, 1000.0f);
}
