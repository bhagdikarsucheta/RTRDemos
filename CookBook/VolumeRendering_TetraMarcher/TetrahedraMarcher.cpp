...
void TetrahedraMarcher::SetVolumeDimension(const int xdim, const int ydim, const int zdim)
{
	XDIM = xdim;
	YDIM = ydim;
	ZDIM = zdim;
...
void TetrahedraMarcher::SetIsoSurfaceVaule(const GLubyte value)
{
	isoValue = value;
}
...
void TetrahedraMarcher::SampleVoxel(const int x, const int y, const int z, glm::vec3 scale)
{
	...
	//obtain a fagIndex based on if the value at the cube vertex is less
	//than given isovalue
	...
	//if the cube is entirely inside or outside of the usrface, then there will be no intersections
	...
	for (i = 0; i < 12; i++)
	{
		...
		if (edgeFlags & (1 << i))
		{
			...
			//use the offset to get the vertex position
